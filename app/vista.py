...
class Vista(ctk.CTk):
    ...
    def mostrar_sql_server_to_sql_server(self):
        ...
        self.__limpia_frame(self.contenedor_contenido)
        
        self.color = "#23518C"
        self.label_contenido = ctk.CTkLabel(
            self.contenedor_contenido,
            text='Migración de SQL Server a SQL Server',
    ...
    def mostrar_hive_to_sql_server(self):
        ...
        self.label_contenido = ctk.CTkLabel(
            self.contenedor_contenido,
            text='Migracion de Hive a SQL Server',
            font=('Segoe UI', 16, 'bold'),
            bg_color=self.color,
            justify="center"
        )
        self.label_contenido.grid(row=0, column=0, columnspan=4, padx=10, pady=10, sticky="nsew")
        self.__plantilla_form("hivexsql_server")
        # Etiqueta y campo para dns origen
        label_dns_orig = self.__label_form("URL:", 12)
        label_dns_orig.grid(row=5, column=0, padx=10, pady=10, sticky='e')
        self.dns_orig = self.__entry_form('localhost:10000/default;ssl=False;sslTrustStore=file.jks')
        self.dns_orig.grid(row=5, column=1, padx=10, pady=10)
    ...
    def mostrar_oracle_to_sql_server(self):
        ...
        self.label_contenido = ctk.CTkLabel(
            self.contenedor_contenido,
            text='Migracion de Oracle a SQL Server',
            font=('Segoe UI', 16, 'bold'),
            bg_color=self.color,
            justify="center"
        )
    ...
    def __limpia_frame(self, frame: ctk.CTkFrame):
        for widget in frame.winfo_children():
            widget.destroy()
    
    def __plantilla_form(self, clave: str):
        # Label de origen
        ...
        self.usr_orig = self.__entry_form('sa')
        ...
        self.usr_dst = self.__entry_form('sa')
        ...
        self.db_dst = self.__entry_form('master')
